...
session_start(); //we need session for the log in thingy XD
    ...
    $id = $_GET['id'];
    $query = "select * from `archive_table` where `id` = '$id'; ";
    if(count(fetchAll($query)) > 0){
        foreach(fetchAll($query) as $row){
          ...
          $cat_id = $row['cat_id'];
          $date = $row['date'];
          $query = "INSERT INTO `work_table` (`id`, `work_title`, `website_link`, `roles_services`, `credits_design`, `credits_copy`, `location_year`, `wallpaper`, `website_video`, `mobile1_website_image`, `mobile2_website_image`, `mobile_website_video`, `tablet_website_video`, `tablet_website_section_video`, `cat_id`, `date`)
        ...
        $query .= "DELETE FROM `archive_table` WHERE `archive_table`.`id` = '$id';";
        if(performQuery($query)){
          $msg = "<div class='alert alert-danger'>$work_title has been enabled and saved at work.</div>";
